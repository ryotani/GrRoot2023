...
void timecor(char* filename, char* cutname, int afp, int iter = 0);
void afpcor(char* filename, char* cutname, float ustart, float ustop, int usteps, int iter = 0);
void momcor(int type, float ustart, float ustop, int usteps);
void iicor(int type);
void iicor_pro(int type);
void xcor(char* filename, char* cutname, float ustart, float ustop, int usteps, int iter = 0);
void reset();
...
void corrections(){
  ...
  cout << "  \"afpcor(\"FILENAME\", \"CUTNAME\", float \"FROM\", float \"TO\", int \"STEPS\", int \"ITER\")\"" << endl;
  cout << "  (you should run this before the x correction and\n  apply the new settings after it.)" << endl;
  cout << "  \"xcor(\"FILENAME\", \"CUTNAME\", float \"FROM\", float \"TO\", int \"STEPS\", int \"ITER\")\"" << endl;
...
void afpcor(char* filename, char* cutname, float ustart, float ustop, int usteps, int iter){
  steps = usteps;
  start = ustart;
  inc = (ustop-ustart)/usteps;
  timecor(filename, cutname, 1, iter);
  fiter = iter;
}
void xcor(char* filename, char* cutname, float ustart, float ustop, int usteps, int iter){
  steps = usteps;
  start = ustart;
  inc = (ustop-ustart)/usteps;
  timecor(filename, cutname, 0, iter);
  fiter = iter;
}
void timecor(char* filename, char* cutname, int afp, int iter){
  ...
  if(afp>0){
    if(iter == 0){
      h[0] = (TH2F*)f->Get(Form("afp_vs_objm_%s",cutname));
      h[1] = (TH2F*)f->Get(Form("afp_vs_xfpm_%s",cutname));
      h[2] = (TH2F*)f->Get(Form("afp_vs_objtac_%s",cutname));
      h[3] = (TH2F*)f->Get(Form("afp_vs_xfptac_%s",cutname));
    }
    else{
      h[0] = (TH2F*)f->Get(Form("afp_vs_objmC_%s",cutname));
      h[1] = (TH2F*)f->Get(Form("afp_vs_xfpmC_%s",cutname));
      h[2] = (TH2F*)f->Get(Form("afp_vs_objtacC_%s",cutname));
      h[3] = (TH2F*)f->Get(Form("afp_vs_xfptacC_%s",cutname));
    }
  }
  else{
    if(iter == 0){
      h[0] = (TH2F*)f->Get(Form("x_vs_objm_%s",cutname));
      h[1] = (TH2F*)f->Get(Form("x_vs_xfpm_%s",cutname));
      h[2] = (TH2F*)f->Get(Form("x_vs_objtac_%s",cutname));
      h[3] = (TH2F*)f->Get(Form("x_vs_xfptac_%s",cutname));
    }
    else{
      h[0] = (TH2F*)f->Get(Form("x_vs_objmC_%s",cutname));
      h[1] = (TH2F*)f->Get(Form("x_vs_xfpmC_%s",cutname));
      h[2] = (TH2F*)f->Get(Form("x_vs_objtacC_%s",cutname));
      h[3] = (TH2F*)f->Get(Form("x_vs_xfptacC_%s",cutname));
    }
  }
  /*
  if(afp>0){
  ...
  */
  ...
  for(int j=0;j<4;j++){
	...
	for(int y=1;y<ny;y++){//afp
	  ...
	  if(h[j]->GetBinContent(x,y)>0){
	    // if(afp==0 && y>from)
	    //   cout << y <<"\t"<<h[j]->GetBinContent(x,y)<< endl;
	    // if(afp==0 && y>from && y < to)
  ...
  for(int j=0;j<4;j++){
   ...
   c1->cd(1+j);
    ...
    c1->cd(5+j);
    hres[j]->Draw("colz");
    hres[j]->GetXaxis()->SetRangeUser(fitlow[j],fithigh[j]);
  }
  if(afp>0){
    OBJe1 = result[0];
    XFPe1 = result[1];
...
void iccor(char* filename){
  ...
  //adjust here!
  TH2F *h = (TH2F*)f->Get("ICde_vs_x0_in78Rbout76Kr");
  ...
  p->GetYaxis()->SetRangeUser(700,900);
  TF1 *fu = new TF1("fu",fexpo,-300,300,3);
  TH2F *hres;

  //adjust here!
  fu->SetParameter(2,2010); //x0
  fu->SetParameter(1,0.0002); //p
  fu->SetParameter(0,-0.001); //E
  ...
  //adjust here!
  TH2F *unc = (TH2F*)f->Get("ICde_vs_x0_in78Rb");

  int nx = unc->GetNbinsX();
  int lx = unc->GetXaxis()->GetBinLowEdge(1);
  int ux = unc->GetXaxis()->GetBinUpEdge(unc->GetNbinsX());
  int ny = unc->GetNbinsY();
  int ly = unc->GetYaxis()->GetBinLowEdge(1);
  int uy = unc->GetYaxis()->GetBinUpEdge(unc->GetNbinsY());
  TH2F *all = new TH2F("all","all",nx,lx,ux,ny,ly,uy);
  for(int x=1;x<nx;x++){
    double xp = unc->GetXaxis()->GetBinCenter(x);
    for(int y=1;y<ny;y++){
      double yp = unc->GetYaxis()->GetBinCenter(y);
      if(unc->GetBinContent(x,y)>0){
	if(xp<ICX0Corr)
	  all->Fill(xp,yp*exp(ICXCorr*(ICX0Corr-xp)),unc->GetBinContent(x,y));
	else
	  all->Fill(xp,yp,unc->GetBinContent(x,y));
      }
    }
  }
  all->Draw("colz");
