...
TFile *f = new TFile("/shared/storage/physnp/mab503/GrROOT_Files/Calculate_out/47Ti/47Titest.root");

  TH1F *h[2];
  h[0] = (TH1F*)f->Get(Form("egamdc_gretthetaback_%s",name));
  h[1] = (TH1F*)f->Get(Form("egamdc_gretthetafront_%s",name));
  h[0]->Rebin(2);
  h[1]->Rebin(2);
  h[0]->Draw("");
  h[0]->SetMinimum(0);
