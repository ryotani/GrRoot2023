...
void Settings::ReadSettings(TEnv* set){
  char* defaultfile = (char*)"/shared/storage/physnp/rdol500/e16016/BetaChain/NewMethod/89TimeCorr.dat";
  ...
  fm_objE1C = set->GetValue("MOBJ.afp.Corr",0.0); //added by ryan (other bit seems to use old values that aren't used)
  fm_objC = set->GetValue("MOBJ.xfp.Corr",0.0);
  fm_xfpE1C = set->GetValue("MXFP.afp.Corr",0.0);
  fm_xfpC = set->GetValue("MXFP.xfp.Corr",0.0);
  //if (fm_objE1C==0.0)
    //fm_objE1C = set->GetValue("MOBJe1.Corr",0.0);
  //fm_objC = set->GetValue("MOBJ.xfp.Corr",0.0);   commented out by ryan
  //if (fm_objC==0.0)
    //fm_objC = set->GetValue("MOBJ.Corr",0.0);
  //fm_xfpE1C = set->GetValue("MXFP.afp.Corr",0.0);
  //if (fm_xfpE1C==0.0)
    //fm_xfpE1C = set->GetValue("MXFPe1.Corr",0.0);
  //fm_xfpC = set->GetValue("MXFP.xfp.Corr",0.0);
  //if (fm_xfpC==0.0)
    //fm_xfpC = set->GetValue("MXFP.Corr",0.0);
  ...
  //fClusterAngle = set->GetValue("ClusterAngle",15);
  ...
  fdet2hole.clear();
  fhole2det.clear();
  for(int det=0; det<11; det++){   //should this be 11? would explain egam_summary going to 32: (7*4)+4
  ...
  fMTDCrefchan = set->GetValue("MTDC.RefChannel",15);
  fMTDC_XFPgate[0] = set->GetValue("MTDC.XFPGate.low",-1000);
  fMTDC_XFPgate[1] = set->GetValue("MTDC.XFPGate.high",-1000);
  fMTDC_OBJgate[0] = set->GetValue("MTDC.OBJGate.low",-1000);
  fMTDC_OBJgate[1] = set->GetValue("MTDC.OBJGate.high",-1000);

  fobj_range[0] = set->GetValue("OBJ.NBins",500);
  fobj_range[1] = set->GetValue("OBJ.Low",0);
  fobj_range[2] = set->GetValue("OBJ.High",4000);
  fOBJ_Offset = set->GetValue("OBJ_Offset",0);
  fXFP_Offset = set->GetValue("XFP_Offset",0);
  ...
  fmobj_range[0] = set->GetValue("MOBJ.NBins",500);
  fmobj_range[1] = set->GetValue("MOBJ.Low",0);
  fmobj_range[2] = set->GetValue("MOBJ.High",4000);
  fmxfp_range[0] = set->GetValue("MXFP.NBins",500);
  fmxfp_range[1] = set->GetValue("MXFP.Low",0);
  fmxfp_range[2] = set->GetValue("MXFP.High",4000);
  ...
  fmobjC_range[0] = set->GetValue("MOBJ.Corr.NBins",500);
  fmobjC_range[1] = set->GetValue("MOBJ.Corr.Low",0);
  fmobjC_range[2] = set->GetValue("MOBJ.Corr.High",4000);
  fmxfpC_range[0] = set->GetValue("MXFP.Corr.NBins",500);
  fmxfpC_range[1] = set->GetValue("MXFP.Corr.Low",0);
  fmxfpC_range[2] = set->GetValue("MXFP.Corr.High",4000);
...
void Settings::PrintSettings(){
  ...
  cout << "MTDC.RefChanne\t" << fMTDCrefchan << endl;

  cout << "MTDC.XFPGate.low\t" << fMTDC_XFPgate[0] << endl;
  cout << "MTDC.XFPGate.high\t" << fMTDC_XFPgate[1] << endl;
  cout << "MTDC.OBJGate.low\t" << fMTDC_OBJgate[0] << endl;
  cout << "MTDC.OBJGate.high\t" << fMTDC_OBJgate[1] << endl;
  //cout << "\t" << f << endl;
  //cout << "\t" << f << endl;
