...
void Calibration::BuildAllCalc(S800* inS800, Gretina* inGret, Mode3Event* inMode3,
  ...
  if(hass800){
   ...
   //cout << "pass" <<  outS800->GetTRACK()->GetTheta() << endl;  
}
  if(hasgret){
...
void Calibration::BuildS800Calc(S800* in, S800Calc* out){
  ...
  //TOF
  BuildTof(in->GetTimeOfFlight(),&track,&tof);

  BuildMTDC(in->GetMesyTDC(),&mtdc);
...
void Calibration::BuildTrackCalc(Float_t x0, Float_t y0, Float_t x1, Float_t y1,
  ...
  if(order > 0){
   ...
   // cout << "AFP" << atan((x1 - x0)/gap) * 1000 <<  " XFP " << (x0 + zfp * tan(out->GetAFP()/1000)) << " YFP " << (y0 + zfp * tan(out->GetBFP()/1000)) <<  " BFP " << (atan((y1 - y0) / gap) * 1000) <<endl;
    //cout << "out->fata " << out->GetATA() << " out->fyta " << out->GetYTA() << " out->fbta " << out->GetBTA() << " out->fdta " << out->GetDTA() << endl;
   ...
   if(fabs(out->GetYTA()) > 100)
    ...
    out->SetYTA(out->GetYTA()+fSett->ShiftY()); //added by Ryan ("previously no fSett->ShiftY()" meaning that YTA offset wasn't being applied)
    ...
    out->SetPhi(s800_phi);//in rad


    //scatter polar angle at traget = theta

    out->SetTheta(asin(sqrt(xsin*xsin + ysin*ysin)));//in rad
...
void Calibration::CorrectTrack(TRACK* tr, IITRACK* ii){
  double dta = tr->GetDTA();
  if(dta>10 || dta <-10)
    tr->SetDTAC(sqrt(-1.0));
  else if(ii->GetXII()<-50 || ii->GetXII()>50)
    tr->SetDTAC(sqrt(-1.0));
  else if(!isnan(ii->GetXII()))
    tr->SetDTAC(dta+ii->GetXII()*fSett->DTACorr());
  else
    tr->SetDTAC(sqrt(-1.0));
  Float_t ratio, betagam, gamma, p0, e0;
  betagam = fbrho/3.107/fmass*fcharge; //3.107 = 3.33547*0.9315
  //logbook page 4 Brho = 3.335 p in GeV/c
  gamma = sqrt(betagam*betagam+1);
  ratio = gamma/(gamma+1);
  ...
  tr->SetPtotC(p0*(1.0 + tr->GetDTAC()/100*ratio));
  tr->SetPparC(tr->GetPtot()*cos(tr->GetTheta()));
  tr->SetPtraC(tr->GetPtot()*sin(tr->GetTheta()));
  tr->SetEtotC(e0*(1.0 + tr->GetDTAC()/100));
}
...
void Calibration::BuildHodoscope(GHodoscope* in, HODO* out){
  ...
  vector<Double_t> en;
  vector<Double_t> ch;
  out->SetHitPattern(in->GetHitPattern(1)<<16|in->GetHitPattern(0));
  if(fSett->VLevel()>3){
    cout << "hodo hit pattern " << in->GetHitPattern(0) << " = ";
    for(int j=0;j<16;j++){
      if(in->GetHitPattern(0) & (1<<j))
	cout << 1;
      else
	cout << 0;
    }
    cout <<"\t"<< in->GetHitPattern(1) << " = ";
    for(int j=0;j<16;j++){
      if(in->GetHitPattern(1) & (1<<j))
	cout << 1;
      else
	cout << 0;
    } 
    cout << endl;
    cout << "hodo calc hit pattern " << out->GetHitPattern() << " = ";
    for(int j=0;j<32;j++){
      if(out->GetHitPattern() & (1<<j))
	cout << 1;
      else
	cout << 0;
    }
...
Float_t Calibration::MOBJCorrection(TRACK* tr){
  return fSett->MOBJe1C()*tr->GetAFP() + fSett->MOBJC()*tr->GetXFP();
}
Float_t Calibration::MXFPCorrection(TRACK* tr){
  return fSett->MXFPe1C()*tr->GetAFP() + fSett->MXFPC()*tr->GetXFP();
}
...
void Calibration::BuildMTDC(GMesyTDC* in, MTDC* out){
  out->Clear();
  if(fSett->VLevel()>2){
    cout << in->GetNChan() << " channels" << endl;
    for(UShort_t i =0; i<in->GetNChan(); i++){
      cout << "channel " << in->GetChannels()->at(i);
      for(UShort_t j =0; j<in->GetChanMult(i); j++){
	cout << "\t" << in->GetTimes(i)->at(j);
      }
      cout << endl;
    }
  }
  ...
  int reftime=0;
  for(UShort_t i =0; i<in->GetNChan(); i++){
    if(in->GetChannels()->at(i) == fSett->GetMTDCRefChannel()){ // find the reference channel
      refchan = i;
      reftime = in->GetTimes(i)->at(0); // the reference channel should only have one hit, but if there are more, take the first
    }
  }  
  if(refchan==-1 && fSett->VLevel()>0){
    cout << "error MTDC reference channel (set to "<<fSett->GetMTDCRefChannel()<<" in the settings) not found." << endl;
  }
  else{
    for(UShort_t i =0; i<in->GetNChan(); i++){
      if(i==refchan)
	out->Set(in->GetChannels()->at(i),*(in->GetTimes(i)));
      else{
	vector<Int_t> subtracted;
	for(UShort_t j =0; j<in->GetChanMult(i); j++){
	  subtracted.push_back(in->GetTimes(i)->at(j)-reftime);
	}
	out->Set(in->GetChannels()->at(i),subtracted);
      }
    }
  }
  return;
}
void Calibration::BuildMTDCTof(MTDC* in, TRACK* track, TOF* out){
  for(UShort_t i=0; i<in->GetXFP()->size(); i++){
    out->SetMXFPV(in->GetXFP()->at(i));
    out->SetMXFPCorrV(in->GetXFP()->at(i),MXFPCorrection(track));
    if(in->GetXFP()->at(i)>fSett->GetMTDC_XFPGate_low() && in->GetXFP()->at(i)<fSett->GetMTDC_XFPGate_high()){
      out->SetMXFP(in->GetXFP()->at(i)+fSett->XFP_Offset());
      out->SetMXFPCorr(in->GetXFP()->at(i)+fSett->XFP_Offset(),MXFPCorrection(track));
    }
  }
  for(UShort_t i=0; i<in->GetOBJ()->size(); i++){
    out->SetMOBJV(in->GetOBJ()->at(i));
    out->SetMOBJCorrV(in->GetOBJ()->at(i),MOBJCorrection(track));
    if(in->GetOBJ()->at(i)>fSett->GetMTDC_OBJGate_low() && in->GetOBJ()->at(i)<fSett->GetMTDC_OBJGate_high()){
      out->SetMOBJ(in->GetOBJ()->at(i)+fSett->OBJ_Offset());
      out->SetMOBJCorr(in->GetOBJ()->at(i)+fSett->OBJ_Offset(),MOBJCorrection(track));
    }
  }
