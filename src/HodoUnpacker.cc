...
int main(int argc, char* argv[]){
  ...
  while(!feof(infile) && !signal_received){
    ...
    unsigned int length[1];//length in bytes
    bsize = fread(length, sizeof(unsigned int), 1, infile);
    cout << "length\t" << "0x"<<(hex)<< length[0] <<"\t"<<(dec)<< length[0] << endl;
    unsigned int tag[1];
    bsize = fread(tag, sizeof(unsigned int), 1, infile);
    cout << "tag\t" << "0x"<<(hex)<< tag[0] <<"\t"<<(dec)<< tag[0] << endl;
    length[0]-=8; //subtract the already read bytes
    if(tag[0]!=0x1e){
      char crap[1];
      for(int i=0;i<length[0];i++){
	bsize = fread(crap, sizeof(char), 1, infile);
      }
      continue;
    }
    cout << "physics event found " << endl;
    char crap[1];
    for(int i=0;i<length[0];i++){
      bsize = fread(crap, sizeof(char), 1, infile);
    }
    ...
    // //ignore nons800 stuff for s800 evt files
    // unsigned short int crap[1]={0};
    // while(crap[0]!=S800_PACKET)
    //   bsize = fread(crap, sizeof(unsigned short int), 1, infile);
    // cout << "found " << "0x"<<(hex)<< crap[0] <<"\t"<<(dec)<< crap[0] << endl;
    // //return 77;
    
    // //Read the header of the new buffer.
    // unsigned int header[4];
    // unsigned int reader[3];
    // bsize = fread(reader, sizeof(unsigned int), 3, infile);
    // cout << "crap\t0x"<<(hex)<< crap[0] <<"\t"<<(dec)<< crap[0] << endl;
    // // for(int i=0;i<3;i++){
    // //   cout << "0x"<<(hex)<< crap[0] <<"\t"<<(dec)<< crap[0] << endl;
    // // }
    // for(int i=0;i<3;i++){
    //   cout <<"reader["<<i<<"]"<< "0x"<<(hex)<< reader[i] <<"\t"<<(dec)<< reader[i] << endl;
    //   header[i+1] = reader[i];
    // }
    // header[0] = crap[0];
