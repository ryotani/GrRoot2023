...
double HitCalc::DopplerCorrectionFactor(TVector3 PosToTarget, Settings* set, TRACK* track){
  ...
  gamma = 1/sqrt(1-beta*beta);
    cout << "Using Spectcl" << endl;
  
  return gamma*(1-beta*CosDop);

#else

  double xta = 0; //added by ryan: beam spot x offset (XTA) 

  PosToTarget.SetY(PosToTarget.Y() + track->GetYTA());  //ryan: should this be add yta (as in spectcl above?), makes sense that it should be add
  //PosToTarget.SetY(PosToTarget.Y()); //Test with no YTA correction 
  PosToTarget.SetX(PosToTarget.X() - xta); 
  ...
  //BeamDir.RotateX(  (track->GetBTA() - set->GretinaAngleB() )/1000 ); 
  //added by ryan
  //cout << "Gretina Angle A: " << set->GretinaAngleA() << endl;
  //cout << "Gretina Angle B: " << set->GretinaAngleB() << endl;


  double beamthetaoff = 0.00; //added:offset to theta beam
  double thetaoff = 0.00; //added: offset to theta difference between beam and gamma emission
  double phioff = 0.00; //added: offset the phi beam
  ...
  BeamDir.SetPhi(BeamDir.Phi()+phioff);
  BeamDir.SetTheta(BeamDir.Theta()+beamthetaoff);
  //double test= BeamDir.Theta();
  
  double CosDop = cos((PosToTarget.Angle(BeamDir))+thetaoff); //theta offset added
//cout << PosToTarget.Angle(BeamDir) << endl;
  double dphi = track->GetPhi() - PosToTarget.Phi();
	while(dphi<0)
	dphi+=2*TMath::Pi();
