...
int main(int argc, char* argv[]){
  ...
  for(int i=0; i<nentries && !signal_received; i++){
    ...
    for(UShort_t in=0;in<InPartCut.size();in++){
      ...
      if( ((tac==1 || tac==3)&& InPartCut[in]->IsInside(tof->GetOBJ(),tof->GetXFP())) ||
	  ((tac==0 || tac==2)&& InPartCut[in]->IsInside(tof->GetTACOBJ(),tof->GetTACXFP()))||
	  ((tac==4 || tac==5)&& InPartCut[in]->IsInside(tof->GetMOBJ(),tof->GetMXFP())) ){
	...
	for(UShort_t ou=0;ou<OutPartCut[in].size();ou++){
	  if((tac == 0 && OutPartCut[in][ou]->IsInside(tof->GetTACOBJC(),icDE)) ||
	     ...
	     (tac == 4 && OutPartCut[in][ou]->IsInside(tof->GetMXFPC(),icDE)) ||
	     (tac == 5 && OutPartCut[in][ou]->IsInside(tof->GetMOBJC(),icDE))
	     ){
