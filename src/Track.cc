...
int main(int argc, char* argv[]){
  ...
  for(int i=0; i<nentries;i++){
    ...
    //added by ryan
    double tbeam = BeamDir.Theta();
    double pbeam = BeamDir.Phi();
    cout << "Gamma: " << i << "    Theta Beam: " << tbeam << endl;
    cout << "Gamma: " << i << "    Phi Beam: " << pbeam << endl;
	...
	dphi-=2*TMath::Pi();
      hegamdc_dphi->Fill(hit->GetDCEnergy(),dphi); //added
