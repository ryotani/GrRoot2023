...
void RawHistograms::FillS800Histograms(S800* s800){
  ...
  GMesyTDC* mtdc = s800->GetMesyTDC();
  GTimeOfFlight* tof = s800->GetTimeOfFlight();
  int ttime = s800->GetTrigger()->GetS800();
  Fill("tac_vs_tdc_xfp",1000,-1500,500,tof->GetXFP()-ttime,1000,0,1000,tof->GetTACXFP());
  Fill("tac_vs_tdc_obj",1000,-1500,500,tof->GetOBJ()-ttime,1000,0,1000,tof->GetTACOBJ());

  for(UShort_t i =0; i<mtdc->GetNChan(); i++){
    if(mtdc->GetChannels()->at(i)==2){ //xfp
      for(UShort_t j =0; j<mtdc->GetChanMult(i); j++){
	Fill("tac_vs_mtdc_xfp",1000, 0, 1000,tof->GetTACXFP(),1000,15000,25000,mtdc->GetTimes(i)->at(j));
	Fill("tdc_vs_mtdc_xfp",1000,-1500,500,tof->GetXFP()-ttime,1000,15000,25000,mtdc->GetTimes(i)->at(j));
      }
      Fill("tdc_vs_mtdc0_xfp",1000, 0, 1000,tof->GetTACXFP(),1000,15000,25000,mtdc->GetTimes(i)->at(0));
      Fill("tac_vs_mtdc0_xfp",1000,-1500,500,tof->GetXFP()-ttime,1000,15000,25000,mtdc->GetTimes(i)->at(0));
    }
    if(mtdc->GetChannels()->at(i)==3){ //obj
      for(UShort_t j =0; j<mtdc->GetChanMult(i); j++){
	Fill("tac_vs_mtdc_obj",1000, 0, 1000,tof->GetTACOBJ(),1000,15000,25000,mtdc->GetTimes(i)->at(j));
	Fill("tdc_vs_mtdc_obj",1000,-1500,500,tof->GetOBJ()-ttime,1000,15000,25000,mtdc->GetTimes(i)->at(j));
      }
      Fill("tdc_vs_mtdc0_obj",1000, 0, 1000,tof->GetTACOBJ(),1000,15000,25000,mtdc->GetTimes(i)->at(0));
      Fill("tac_vs_mtdc0_obj",1000,-1500,500,tof->GetOBJ()-ttime,1000,15000,25000,mtdc->GetTimes(i)->at(0));
    }
  }
}
