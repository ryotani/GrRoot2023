...
int main(int argc, char* argv[]){
  ...
  int filectr =0;
  cout <<"ATTENTION DUE TO CAPPY SCALER TIMESTAMPS IN THE FIRST PART OF THE DATA WE REMOVED THE FIRST 30 SECONDS!! SCALERS COME BEFORE THE IMPSYCH?!"<<endl;
  cout << nentries << " entries in tree " << endl;
  for(int i=30; i<nentries;i++){
    ...
    for(int j=0;j<scalers;j++){
      ...
      if(sc->GetTS()<previous_ts){
	//cout << "sc->GetTS()\t"<< sc->GetTS()<< "\tprevious_ts\t"<< previous_ts << endl;
	//cout << "filectr\t" << filectr << "\tj\t" << j << "\tprevious[j]\t"<< previous[j] << endl;
