...
double GetFitMean(TH1F* hst){
  ...
  fit = new TF1("fit",fonegaus,0,1500,3);
  fit->SetLineColor(3);
  fit->SetLineWidth(1);
  ...
  fit->SetRange(center-200,center+200);
  fit->SetParLimits(1,center-50,center+50);
...
int main(int argc, char* argv[]){
  ...
  if(namesfound<blobs){
    cout << "Histogram file must have at least "<<blobs<<" outgoing gates. You have " <<namesfound<< endl;
  ...
  for(int c=0;c<2;c++){
    ...
    for(int i=0;i<blobs;i++){
      ...
      if(HOutput){
	HFile->cd();
	fit->SetName(Form("f_%s",h[i]->GetName()));
	h[i]->Write();
	fit->Write();
      }
