...
int UnpackedEvent::DecodeS800(unsigned short *pevent, long long int ts, unsigned short twords){
  ...
  while(nwords > 0){
    ...
    case S800_MESYTDC_PACKET:
      if(fvl>0)
	cout << "UnpackedEvent: " << "MESY TDC " << endl;
      if(plength-2>0)
	p = DecodeS800MTDC(p,plength-2);
      break;
...
unsigned short* UnpackedEvent::DecodeS800MTDC(unsigned short *p, unsigned short length){
  if(fvl>2){
  ...
  while(length > 0){
    ...
    *p++;
    int data = *p++;
    if(fvl>2){
      cout << "\tch\t" << ch  << ", " << (dec)<< ch  << "\thit\t" << (hex)<< hit << ", " << (dec) << hit << endl;
      cout << "data\t" << (hex)<< data << ", " << (dec) << data << endl;
    }
    length -= 2;
    if(ch == prevch){
      ...
      fS800->GetMesyTDC()->Set(prevch,times);
      times.clear();
      times.push_back(data);
      prevch = ch;
    }
  ...
  fS800->GetMesyTDC()->Set(prevch,times);
  if(fvl>2){
    cout << fS800->GetMesyTDC()->GetNChan() << " channels" << endl;
    for(UShort_t i =0; i<fS800->GetMesyTDC()->GetNChan(); i++){
      cout << "channel " << fS800->GetMesyTDC()->GetChannels()->at(i);
      for(UShort_t j =0; j<fS800->GetMesyTDC()->GetChanMult(i); j++){
	cout << "\t" << fS800->GetMesyTDC()->GetTimes(i)->at(j);
      }
      cout << endl;
    }
  }
